...
**Why preview generation can take a long time:** Preview runs as a **background task** (render blueprint → upload bundle + thumbnail). On free-tier hosting (e.g. Render), the service may cold-start and the job can take 60–120 seconds. The UI polls for up to 2 minutes; if the job doesn’t finish in time you’ll see “Preview is taking longer than expected.” Use **Reset preview** then **Generate Preview** again, or ensure the backend stays warm. Optional: set `PREVIEW_SYNC_TIMEOUT` (default 90s) and use sync mode in the API if you want the request to wait for completion.
