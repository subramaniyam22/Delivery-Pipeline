...
**Create Template wizard:** Name, description, category, style, industry, feature tags; **Image prompts (optional)** for Exterior, Interior, Lifestyle, People, Neighborhood — short descriptions that guide AI when generating or selecting images for the template. Leave blank to skip. **Initial template ZIP (optional):** In step 4 you can upload a .zip of your template files; after the template is created it is uploaded to S3 and set as that version's build source (`s3_zip`). For replacing or updating a version's build source later (e.g. hotfixes), use the template's **Versions** tab.

**Multi-page preview:** Preview is generated as **multiple HTML pages** (e.g. `index.html`, `about.html`, `contact.html`). Every link in the nav and footer is given a real page; if the blueprint has no page for a slug, a placeholder page is generated so **no link leads to 404**. Preview is served via the API proxy so navigation between pages works.

**Sample maps:** The location map section in the preview uses an **OpenStreetMap** embed (no API key). Coordinates come from the demo/client `property.geo` or a default location so the preview shows a real map in context.
...
**Why opening the template URL in a new tab returns 401:** `GET /api/templates/{id}` (and most API routes) require a **Bearer JWT**. The app sends the token in the `Authorization` header when you're logged in. Opening the same URL in a new tab or incognito sends no token, so the server correctly returns `401 Unauthorized`. This is expected and is not the cause of preview slowness.

**Why preview generation can take a long time / stay “Generating”:** Template preview is **queued to the worker** (generic job `template.preview.generate`). The worker process must be running to pick up and run the job; if only the web service is running (e.g. Render free tier with no worker), the job never runs and the template stays “Generating”. **Fix:** Ensure the **worker** service is deployed and running, then use **Reset preview** and **Generate Preview** again. The UI polls for up to 2 minutes; if the worker is slow or cold, use **Reset preview** and retry. Optional: set `PREVIEW_SYNC_TIMEOUT` (default 90s) and call the API with `sync: true` to run preview in the web request (keeps the process alive until done or timeout).

**Versioned template storage (clone template):** Templates can reference an immutable build source for reproducible builds. Each template row may set `build_source_type` (`s3` | `s3_zip` | `git`) and `build_source_ref` (S3 key from `build_template_key(template_id, version)` or git commit/tag ref). You can provide an **initial template ZIP** when creating the template (Create Template wizard); the **Versions** tab is for replacing or updating a version's build source (e.g. hotfixes). **TemplateArtifact** records immutable template zips in S3 (s3_key, version, checksum). When a client selects a template, a **ProjectTemplateInstance** is created for that project (with optional fallback). On BUILD stage start, the **worker** (site_builder): (1) downloads template zip from S3 (or clones git ref), (2) unzips into temp dir, (3) injects client data (`data/client_contract.json`), (4) runs build and **requires dist/index.html** (fail-fast before package/upload; error lists top-level dirs if missing), (5) uploads to **previews/{project_id}/{run_id}/site/** (tagged `expires=30d`) and copies to **deliveries/{project_id}/current/site/**. Preview and delivery URLs use **CloudFront** via `get_preview_url()` and `get_delivery_url()`. Proof packs upload to **artifacts/** with tag `expires=180d`. If the selected template is missing, the system can create a **ConfirmationRequest** and block until the client approves/rejects a fallback (portal or **POST /confirmations/{id}/approve** or **/reject**); reminder email sent daily (max 10).
